...
late String _prim_email ;

class EventCalendar extends StatefulWidget {
  final String mail;
  const EventCalendar({Key? key, required this.mail}) : super(key: key);

  @override
  EventCalendarState createState() => EventCalendarState(mail: this.mail);
}
...
class EventCalendarState extends State<EventCalendar> {
  final String mail;
  EventCalendarState({Key? key, required this.mail}) ;
  // EventCalendarState(this.mail);
  ...
  Future<void> getDataFromFireStore() async {
    var snapShotsValue = await databaseReference
        .collection("CalendarAppointmentCollection").doc(_prim_email)
        .get();
    ...
    while(i<sz){
      list.add(
          Meeting(
              ...
              background: _colorCollection[random.nextInt(9)], //_colorCollection[_selectedColorIndex],
    ...
    // List<Meeting> list = data.entries.map((e) =>
    //     (e.key == 'Email' ? em = e.value)
    // ).toList(); Meeting(
    //   email: data()['Email'],///////////////////////////
